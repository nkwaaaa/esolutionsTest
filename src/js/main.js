...
function obtenerDatosDelUsuario() {
  ...
  datosPersona.aguantePrompt = confirm(
    "¿Le gusta responder preguntas en un prompt en lugar de una librería que simplifique y mejore las cosas?"
  );
}
...
function renderizarDatosUsuario() {
  ...
  datosPersona.aguantePrompt
    ? (aguantePrompt.innerText = "Sí, le queda bien lo rústico")
    : (aguantePrompt.innerText = "No Matías, ya quedaste re mal");
}
...
const listadoRespuestas =
  JSON.parse(localStorage.getItem("respuestasAlmacenadas")) || [];
...
function guardarEnLocal() {
  localStorage.setItem(
    ...
    JSON.stringify(listadoRespuestas)
  );
}
